...
export class ApexonDashboard implements vscode.WebviewViewProvider {
  ...
  private async handleMessage(msg: any) {
    switch (msg.command) {
      ...
      case 'run':  await this.doRun(msg.baseURL, msg.apiKey, msg.indices, msg.overrides); break;
      ...
      case 'openFile': await this.doOpenFile(msg.file, msg.line); break;
    }
  }

  private async doOpenFile(filePath: string, line?: number) {
  ...
  private async doRun(baseURL: string, apiKey: string, indices: number[], overrides: Record<string, Record<string, string>> = {}): Promise<any[]> {
